...
function IsValidDecimalRange(strValue) {
    ...
    var REG_EX_REAL_DELIMITER = "[:]",
        aryNumericRangeSplit, strMin, strMax;
    strValue = new String(strValue);
...
(function() {
    ...
    LRS.prototype = {
        ...
        _processGetStatementResult: function(xhr, params) {
            var boundary, parsedResponse, statement, attachmentMap = {},
                i;
        ...
        saveStatement: function(stmt, cfg) {
            ...
            var requestCfg = {
                ...
                versionedStatement, requestAttachments = [],
                boundary, i;
            cfg = cfg || {};
            try {
        ...
        retrieveStatement: function(stmtId, cfg) {
            ...
            var requestCfg, requestResult, callbackWrapper, lrs = this;
            cfg = cfg || {};
            cfg.params = cfg.params || {};
            requestCfg = {
                url: "statements",
        ...
        saveStatements: function(stmts, cfg) {
            ...
            var requestCfg = {
                ...
                versionedStatement, versionedStatements = [],
                requestAttachments = [],
                boundary, i, j;
            cfg = cfg || {};
            if (stmts.length === 0) {
        ...
        queryStatements: function(cfg) {
            ...
            if (typeof cfg.callback !== "undefined") {
                callbackWrapper = function(err, xhr) {
                    var result = xhr,
                        parsedResponse, boundary, statements, attachmentMap = {},
                        i;
                    if (err === null) {
                        if (!cfg.params.attachments) {
        ...
        _parseMultipart: function(boundary, response) {
            var __boundary = "--" + boundary,
                byteArray, bodyEncodedInString, fullBodyEnd, sliceStart, sliceEnd, headerStart, headerEnd, bodyStart, bodyEnd, headers, body, parts = [],
                CRLF = 2;
...
(function() {
    ...
    Attachment.prototype = {
        ...
        init: function(cfg) {
            this.log("init");
            var i, directProps = ["contentType", "length", "sha2", "usageType", "display", "description", "fileUrl"];
            cfg = cfg || {};
            for (i = 0; i < directProps.length; i += 1) {
                if (cfg.hasOwnProperty(directProps[i]) && cfg[directProps[i]] !== null) {
                    this[directProps[i]] = cfg[directProps[i]];
...
(function() {
    ...
    var LOG_SRC = "Environment.Browser",
        requestComplete, __IEModeConversion, nativeRequest, xdrRequest, __createJSONSegment, __createAttachmentSegment, __delay, env = {},
        log = TinCan.prototype.log;
        ...
        this.handler = function(stream, bite) {
            switch (iso2022jp_decoder_state) {
                ...
                case states.EscapeStart:
                    ...
                    stream.prepend(bite);iso2022jp_output_flag = false;iso2022jp_decoder_state = iso2022jp_decoder_output_state;
                    return decoderError(fatal);
                case states.Escape:
                        var lead = iso2022jp_lead;iso2022jp_lead = 0x00;
                    var state = null;
                    ...
                    stream.prepend([lead, bite]);iso2022jp_output_flag = false;iso2022jp_decoder_state = iso2022jp_decoder_output_state;
                    return decoderError(fatal);
            }
        };
    }
...
(function() {
    ...
    TinCan.LRS.prototype._initByEnvironment = function(cfg) {
        ...
        this._makeRequest = function(_fullUrl, headers, cfg) {
            ...
            if (origResult.err === null) {
                return origResult;
            }
            if (typeof origResult.err !== "undefined" && origResult.err === 0) {
                for (prop in cfg.params) {
                    if (cfg.params.hasOwnProperty(prop)) {
                        pairs.push(prop + "=" + encodeURIComponent(cfg.params[prop]));
                    }
                ...
                try {
                    fetch(fullUrl, fetchRequestCfg).then(function(response) {
...
function TCAPI_RecordTrueFalseInteraction(strID, blnResponse, blnCorrect, blnCorrectResponse, strDescription, intWeighting, intLatency, strLearningObjectiveID, dtmTime) {
    WriteToDebug("In TCAPI_RecordTrueFalseInteraction strID=" + strID + ", blnResponse=" + blnResponse + ", blnCorrect=" + blnCorrect + ", blnCorrectResponse=" + blnCorrectResponse + ", strDescription=" + strDescription + ", intWeighting=" + intWeighting + ", intLatency=" + intLatency + ", strLearningObjectiveID=" + strLearningObjectiveID + ", dtmTime=" + dtmTime);
    var strResponse = null,
